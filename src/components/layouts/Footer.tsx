  ...
  return (
    ...
    >
      ...
      <section className={cn(
        "w-full pt-5 pb-5.5 relative bg-[linear-gradient(180deg,#fffcf7_0%,#f7f4ee_100%)]",
        ...
        <p className="text-[11px] uppercase mb-3 font-semibold tracking-[0.08em] text-gold-deep text-center">
          공식 파트너
        </p>
          ...
          >
            <div className="flex lg:gap-2.5 gap-2.5 pr-2.5 bg-transparent">
              {PARTNER_LOGOS.map((logo) => (
                <div key={logo.alt}
                  className={cn("min-w-28 shrink-0 flex items-center justify-center h-14 rounded-lg py-2 px-4 border border-line",
                    "bg-cream"
                  )}
                >
                  <Image
      ...
      <div className={cn("w-full flex flex-col gap-5 text-center",
        "items-center pt-6 px-4 pb-8 text-gray"
      )}>
        ...
        {/* <div className={cn("flex flex-wrap items-center justify-center gap-x-6 gap-y-4")}>
          <Link
            ...
            className="inline-flex hover:text-gold-deep opacity-90 items-center transition-[color,opacity] hover:opacity-100"
          >
          ...
          <Link
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="text-xs underline font-semibold underline-offset-[3px] hover:text-gold-deep transition-colors "
          >
            도박에 책임감을 가지세요
          </Link>
          <span
            className={cn("inline-flex size-9 rounded-full items-center justify-center px-2 border",
              "border-[rgba(198,161,91,0.45)] text-xs text-gold-deep font-bold bg-[#c6a15b0f]"
            )}
          >
            18+
          </span>
        ...
        <div className="flex items-center flex-wrap justify-center gap-2">
          {FOOTER_TAGS.map((tag) => (
            <span
              key={tag.label}
              className={cn(
                "rounded-md inline-flex items-center font-medium border border-line bg-cream px-2.5 h-6.5",
        ...
        <Link href="/" className="mt-1 flex items-center justify-center">
          <Image
            ...
            width={140}
            height={40}
            className="h-11 w-auto rounded-md bg-[#1a1614] object-contain px-1.5"
            unoptimized
          />
        </Link>
      </div>
